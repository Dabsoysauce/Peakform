...
export default function DirectMessagesScreen() {
  ...
  useEffect(() => { if (activeChat) dispatch(fetchDMs(activeChat)); }, [activeChat]);
  useEffect(() => { if (search.length > 1) dispatch(fetchContacts(search)); }, [search]);
    ...
    return (
      <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
        <View style={styles.chatHeader}>
          <TouchableOpacity onPress={() => setActiveChat(null)}><Text style={styles.backBtn}>←</Text></TouchableOpacity>
        ...
        <View style={styles.inputRow}>
          <TextInput style={styles.chatInput} value={input} onChangeText={setInput} placeholder="Type a message..." placeholderTextColor={colors.textMuted} onSubmitEditing={handleSend} />
  ...
  return (
    <View style={styles.root}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => setShowSearch(!showSearch)}><Text style={styles.newBtn}>+</Text></TouchableOpacity>
      </View>
      {showSearch && (
        <View style={{ paddingHorizontal: spacing.md }}>
          <TextInput style={styles.searchInput} value={search} onChangeText={setSearch} placeholder="Search people..." placeholderTextColor={colors.textMuted} autoFocus />
          {contacts.map(c => (
            <TouchableOpacity key={c.id} style={styles.contactItem} onPress={() => startChat(c)}>
              <View style={styles.contactAvatar}><Text style={styles.contactAvatarText}>{(c.first_name?.[0] || c.email[0]).toUpperCase()}</Text></View>
              <View><Text style={styles.contactName}>{c.first_name ? `${c.first_name} ${c.last_name || ''}` : c.email}</Text><Text style={styles.contactRole}>{c.role}</Text></View>
            </TouchableOpacity>
          ))}
        </View>
      )}
      {loading && conversations.length === 0 ? (
        ...
        <View style={styles.empty}><Text style={styles.emptyText}>No conversations yet</Text><Text style={styles.emptySubtext}>Tap + to message a player</Text></View>
      ) : (
        <FlatList
          data={conversations}
          keyExtractor={i => i.partner_id.toString()}
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.convoCard} onPress={() => { setActiveChat(item.partner_id); setActiveName(item.partner_name); }}>
              {item.partner_photo ? <Image source={{ uri: item.partner_photo }} style={styles.convoAvatar} /> : (
                <View style={styles.convoAvatarPlaceholder}><Text style={styles.convoAvatarText}>{(item.partner_name?.[0] || '?').toUpperCase()}</Text></View>
              )}
              <View style={styles.convoInfo}><Text style={styles.convoName}>{item.partner_name}</Text><Text style={styles.convoLast} numberOfLines={1}>{item.last_message}</Text></View>
